...
6. HDFS port forwaring is set up (8020) but does not work - probably because Hadoop binds to a different hostname and drops connections from outside, even with the firewall down. 
A quick workaround is to use SSH tunneling: ```ssh -L 8020:localhost:8020 vagrant@localhost -p 2222``` (password: vagrant)
